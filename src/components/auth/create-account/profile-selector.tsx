...
const ProfileTypeSelector: React.FC = () => {
    ...
    const handleSelectChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
        const selectedType = profileTypes.find((profile) => profile.type === event.target.value);
        if (selectedType) {
            router.push(`${pathname}?${createQueryString(searchParams, "type", selectedType.type)}`);
        }
    };
    ...
    if (width > 992) {
            ...
            <div className="flex flex-col pl-7 mt-20 max-md:pl-5 max-md:mt-10 w-full">
                ...
                {profileTypes.map((type, index) => (
                    <button
                        ...
                        onClick={() => {
                            router.push(
                                `${pathname}?${createQueryString(searchParams, "type", `${type.type}`)}`
                            );
                        }}
        ...
        return (
            <div className="relative w-full my-6">
                ...
                >
                    {profileTypes.map((type, index) => (
                        <option key={index} value={type.type} selected={type.type == activeType} className="flex items-center gap-1.5 text-base">
                            {type.title}
                        </option>
                    ))}
                </select>
