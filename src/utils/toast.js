    ...
    <div className="flex overflow-hidden flex-col w-full bg-white rounded-2xl">
      <div className="flex gap-10 items-start px-4 pt-2.5 pb-6 w-full">
        <div className="flex flex-auto gap-5 mt-4">
          <img loading="lazy" src={avatarSrc} alt="User avatar" className="object-contain shrink-0 w-14 rounded-full aspect-[0.98]" />
          <p className="my-auto">Verification Successful</p>
        </div>
        <img loading="lazy" src={iconSrc} alt="" className="object-contain shrink-0 w-6 aspect-square" />
      </div>
    </div>
  </section>);
